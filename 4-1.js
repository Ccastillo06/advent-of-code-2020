...
const mandatoryLength = mandatoryFields.reduce(
...
const getKeysInRow = (row) => row.replace(/:(\w|#)*(\s|)/g, '');
const removeCid = (row) => row.replace(cid, '');

const filterValidRow = (row) => row.length >= mandatoryLength;

const run = (input) => {
  const formatted = formatInput(input);
  const validRows = formatted.filter((row) =>
    filterValidRow(removeCid(getKeysInRow(row)))
  );
