// Valid output: 184
const initialData = `iyr:2010 ecl:gry hgt:181cm
...
const validByr = (v) => Number(v) >= 1920 && Number(v) <= 2002;
const validIyr = (v) => Number(v) >= 2010 && Number(v) <= 2020;
const validEyr = (v) => Number(v) >= 2020 && Number(v) <= 2030;
const validHgt = (v) => {
  const num = Number(v.replace('cm', '').replace('in', ''));
  return v.includes('cm') ? num >= 150 && num <= 193 : num >= 59 && num <= 76;
};
const validHcl = (v) => /^#[0-9a-f]{6}$/i.test(v);
const validEcl = (v) =>
  'amb' === v ||
  'blu' === v ||
  'brn' === v ||
...
const breakLineJumps = (input) => input.split('\n\n');
const formatRow = (row) => row.split('\n').join(' ');
const formatInput = (input) => breakLineJumps(input).map(formatRow);

const removeCid = (row) => row.replace(/cid:(\w|\d)*(\s|)/, '').trim();
const getKeysInRow = (row) => removeCid(row).split(' ');

const checkValidRow = (row) =>
  row.every((str) => {
    const [key, val] = str.split(':');
    return keyToFn[key](val);
  });
const filterValidRow = (row) =>
  row.length >= mandatoryLength && checkValidRow(row);

const run = (input) => {
  const formatted = formatInput(input);
  const splitRows = formatted.map(getKeysInRow);
  const validRows = splitRows.filter(filterValidRow);

  return validRows.length;
};

const result = run(initialData);
console.log(result);
