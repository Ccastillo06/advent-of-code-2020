...
// PART A
function getMaxMinPerEntry(entry) {
  const [numLimit, letter] = entry.split(' ');
  const [min, max] = numLimit.split('-');
...
function runCodeA(input) {
  const validInputs = input.filter(([key, value]) => {
    const { letter, min, max } = getMaxMinPerEntry(key);

    const hasLetterTimes = value.split(letter).length - 1;
    return hasLetterTimes >= min && hasLetterTimes <= max;
  });
...
// PART B
function getIndexesPerEntry(entry) {
  const [numLimit, letter] = entry.split(' ');
  const [first, second] = numLimit.split('-');

  return {
    letter,
    firstIndex: Number(first) - 1,
    secondIndex: Number(second) - 1,
  };
}

function runCodeB(input) {
  const validInputs = input.filter(([key, value]) => {
    const { letter, firstIndex, secondIndex } = getIndexesPerEntry(key);

    const hasFirstIndex = value[firstIndex] === letter;
    const hasSecondIndex = value[secondIndex] === letter;

    return (
      (hasFirstIndex && !hasSecondIndex) || (hasSecondIndex && !hasFirstIndex)
    );
  });
